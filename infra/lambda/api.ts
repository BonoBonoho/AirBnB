  ...
  try {
    ...
    if (path === '/api/smarthome/command' && method === 'POST') {
      ...
      try {
        ...
        // 사용 기록에 남기기
        const log = (await getDoc<SmartLog>(sub, 'SMARTLOG')) ?? { lastSample: {}, days: {}, events: [] }
        const ev = command === 'setCoolingSetpoint' ? `temp:${arg}`
          : command === 'setAcMode' ? `mode:${arg}`
            : command === 'setFanMode' ? `fan:${arg}`
              : command === 'setVolume' ? `vol:${arg}`
                : command === 'setAcOptionalMode' ? `opt:${arg}`
                  : command
