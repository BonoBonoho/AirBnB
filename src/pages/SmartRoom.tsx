...
export default function SmartRoom() {
  ...
  const assign = async (d: SmartDevice, listingId: string) => {
    ...
    if (listingId) devices.push({ ...d, listingId })
  ...
  return (
    <div>
      ...
      {available.length > 0 && (
        <Card className="mb-4">
          <div className="font-semibold mb-3">발견된 기기 → 숙소에 배정</div>
                ...
                <div key={d.deviceId} className="flex items-center justify-between gap-3 text-sm">
                  <div className="min-w-0 truncate">
                    ...
                    <span className="text-xs text-slate-400 ml-2">
                      {d.provider === 'smartthings' ? '스마트싱스' : 'Tuya'} · {d.caps.map((c) => ({ temp: '온도', humidity: '습도', switch: '스위치', ac: '에어컨' }[c] ?? c)).join('·') || '기타'}
                    </span>
                  </div>
                  <select
          ...
          <Card key={l.id} className="mb-4">
            ...
            <div className="grid md:grid-cols-2 gap-2 mb-4">
              {devices.map((d) => {
                ...
                return (
                  <div key={d.deviceId} className="flex items-center justify-between rounded-xl border border-slate-100 px-3 py-2.5 text-sm">
                    <div>
                      <span className="font-medium">{d.name}</span>
